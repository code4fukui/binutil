...
Deno.test("readUint16", () => {
  const bin = new Uint8Array([
    1, 2, 3, 4, 0xff
  ]);
  const r = new BinReader(bin);
  t.assertEquals(r.readUint16(), 0x0201);
  t.assertEquals(r.readUint16(), 0x0403);
  t.assertEquals(r.readUint8(), 0xff);
});
Deno.test("readUint16 bigendian", () => {
  const bin = new Uint8Array([
    1, 2, 3, 4, 0xff
  ]);
  const r = new BinReader(bin, false);
  t.assertEquals(r.readUint16(), 0x0102);
  t.assertEquals(r.readUint16(), 0x0304);
  t.assertEquals(r.readUint8(), 0xff);
});
